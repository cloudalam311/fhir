 ...
 */
import { stubs, BASE_R4_RESOURCES, BASE_STU3_RESOURCES, } from 'fhir-works-on-aws-interface';
import { ElasticSearchService } from 'fhir-works-on-aws-search-es';
import { RBACHandler } from 'fhir-works-on-aws-authz-rbac';
import { DynamoDb, DynamoDbDataService, DynamoDbBundleService, S3DataService, DynamoDbUtil, } from 'fhir-works-on-aws-persistence-ddb';
...
export const fhirVersion = '4.0.1';
...
], DynamoDbUtil.cleanItem, fhirVersion, loadImplementationGuides('fhir-works-on-aws-search-es'), undefined, { enableMultiTenancy });
const s3DataService = new S3DataService(dynamoDbDataService, fhirVersion, { enableMultiTenancy });
const OAuthUrl = process.env.OAUTH2_DOMAIN_ENDPOINT === '[object Object]' || process.env.OAUTH2_DOMAIN_ENDPOINT === undefined
    ? 'https://OAUTH2.com'
    : process.env.OAUTH2_DOMAIN_ENDPOINT;
export const getFhirConfig = async () => {
    if (enableSubscriptions) {
        const subscriptionAllowList = await getAllowListedSubscriptionEndpoints();
        validators.push(new SubscriptionValidator(esSearch, dynamoDbDataService, subscriptionAllowList, { enableMultiTenancy }));
    }
    return {
        configVersion: 1.0,
        productInfo: {
        ...
        server: {
            ...
            url: process.env.API_URL === '[object Object]' || process.env.API_URL === undefined
                ? 'https://API_URL.com'
                : process.env.API_URL,
        },
        validators,
